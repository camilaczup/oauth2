...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	defer ts.Close()
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, nil, AuthStyleInParams)
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, nil, AuthStyleUnknown)
	if err != nil {
	...
	cancel()
	_, err = RetrieveToken(ctx, clientID, "", cancellingts.URL, url.Values{}, nil, AuthStyleUnknown)
	close(retrieved)
	if err == nil {
